...
if args.model == 'mfcc' or args.model == 'all':
	...
	model = model_.cnn_lstm_mfcc(n_z=args.latent_size, ncoef=args.ncoef, delta=args.delta)
...
if args.model == 'inception_mfcc' or args.model == 'all':
	batch = torch.rand(3, 3 if args.delta else 1, args.ncoef, 200)
	model = model_.inception_v3(n_z=args.latent_size, ncoef=args.ncoef, delta=args.delta)
	mu = model.forward(batch)
	print('inception_mfcc', mu.size())
if args.model == 'resnet_large' or args.model == 'all':
	batch = torch.rand(3, 3 if args.delta else 1, args.ncoef, 200)
	model = model_.ResNet_large_lstm(n_z=args.latent_size, ncoef=args.ncoef, delta=args.delta)
	mu = model.forward(batch)
